    ...
    module Specifications
      class TodoItemDependencySpecification < BaseSpecification
        ...
        def check_dependency_exists!
          ...
          raise Todos::Domain::Errors::ValidationError, 'Dependency not found'
        end

        def check_not_self_dependency!
          return if @todo_item.id != @dependency.id

          raise Todos::Domain::Errors::ValidationError, 'Item cannot depend on itself'
        end

        def check_no_circular_dependency!
          return if !@dependency.depends_on?(@todo_item)

          raise Todos::Domain::Errors::ValidationError, 'Cannot create circular dependency'
        end

        def check_due_date_order!
          return if @todo_item.due_date > @dependency.due_date

          raise Todos::Domain::Errors::ValidationError, "Due date must be after dependency's due date"
        end
      end
    end
  end
